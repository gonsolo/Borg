...
void borgCreateDevice(void) {
  ...
  if (borg_fb_width == 0 || borg_fb_width > BORG_MAX_FB_DIM)
    borg_fb_width = 16;
  if (borg_fb_height == 0 || borg_fb_height > BORG_MAX_FB_DIM)
    borg_fb_height = 16;
...
static void rasterize_clipped_triangle(const clip_vertex_t *cv,
  ...
  if (!triangle_setup(&tri.screen_pos, &tri.inv_area)) return;
