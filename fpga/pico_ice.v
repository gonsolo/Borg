...
module tinyQV_top (
  ...
  always @(*) begin
    if ({addr[27:6], addr[1:0]} == 24'h800000) connect_peripheral = addr[5:2];
    else if (addr[27:11] == 17'h10000) connect_peripheral = PERI_USER;
  ...
  // Debug
  always @(posedge clk) begin
    if (!rst_reg_n) debug_register_data <= ui_in[1];
