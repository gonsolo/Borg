...
void borgCreateGraphicsPipeline(const BorgShaderModule *vert,
  ...
  // Step 29.5: Write sequencer shader binaries to PSRAM and detect hardware.
  // Detection: write 1 to seq_trigger, check if STATUS.seq_busy goes high.
  // If the hardware doesn't have BorgSequencer, seq_trigger is unmapped
  // and seq_busy will stay 0.
  {
    // Write vert shader instructions to PSRAM at SEQ_VERT_SHADER_ADDR
    for (int i = 0; i < (int)vert_shader.num_instrs; i++)
      PSRAM_OUT_RAW(SEQ_VERT_SHADER_ADDR + i * 4) = vert_shader.instrs[i];
    // Write halt after last instruction
    PSRAM_OUT_RAW(SEQ_VERT_SHADER_ADDR + vert_shader.num_instrs * 4) = 0;

    // Write setup shader (triangle_setup_shader from shader_blobs.h)
    // For now, use vert_shader as a placeholder — the actual setup shader
    // is generated by the ISA and will be loaded separately.
    // TODO: embed the 22-instruction setup shader in firmware ROM.

    // Configure sequencer registers (persistent across draw calls).
    BORG_GPU->seq_vert_addr = SEQ_VERT_SHADER_ADDR;
    BORG_GPU->seq_vert_len  = vert_shader.num_instrs + 1; // +1 for halt
    // Setup shader address/length will be set when the setup shader is
    // available in PSRAM.  For now, the sequencer path is disabled:
    // has_sequencer stays 0 (Step 29.5 scaffolding only).
    // TODO: embed the 22-instruction setup shader in firmware ROM,
    // stage it to PSRAM, configure seq_setup_addr/len, then enable
    // auto-detection below.
#if 0
    // Auto-detect: try triggering the sequencer with a dummy descriptor
    // and see if seq_busy goes high.  Use a safe base address (0).
    BORG_GPU->seq_desc_base = 0;
    BORG_GPU->seq_trigger = 1;
    // Check STATUS immediately — seq_busy should be high within 1 cycle.
    volatile uint32_t st = BORG_GPU->status;
    has_sequencer = (st & STATUS_REG_T__SEQ_BUSY_bm) ? 1 : 0;
    if (has_sequencer) {
      // Wait for the dummy sequencer run to complete (it will read garbage
      // from PSRAM address 0, but that's fine — we just need detection).
  ...
  for (int tile_row = 0; tile_row < tile_rows; tile_row++) {
    for (int tile_col = 0; tile_col < tiles_per_row; tile_col++) {
      ...
      for (int slot = 0; slot < count; slot++) {
        ...
        if (has_sequencer) {
          // Trigger the hardware sequencer to stage uniforms autonomously.
          ...
          while (BORG_GPU->status & STATUS_REG_T__SEQ_BUSY_bm)
            ;
          // Sequencer overwrote IMEM with setup shader — reload rast+frag.
          borg_load_spirb_shader_at(&rast_shader, BORG_IMEM_RAST_OFFSET);
          borg_load_spirb_shader_at(&frag_shader, BORG_IMEM_FRAG_OFFSET);
          borg_load_add_shader();
          // Set CONTROL.uniform_write_page to the page the sequencer wrote to.
          // The sequencer toggled from 0→1 (or 1→0); read the current_uniform_page
          // to stay in sync.
          current_uniform_page ^= 1;
          BORG_GPU->control =
              (current_uniform_page << CONTROL_REG_T__UNIFORM_WRITE_PAGE_bp);
