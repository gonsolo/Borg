...
class CsrFile extends Module {
  ...
  // --- External submodules and registers ---
  ...
  // --- Grouped logic execution ---
  updateTimeCounter()
  updateMcauseState()
  updateOutputs()
  updateDoubleFaultState()
  updateMepc()
  updateMachineTrapEnable()
  updateMachineStatus()
  updateInterruptEnableRegisters()
  updateMcauseLogic()
  updateCsrReadMux()

  // -- Private Inline Methods --

  private def updateTimeCounter(): Unit = {
    when(io.counter === 7.U && cycle_cy) {
      time_hi := time_hi + 1.U
    }
  }

  private def updateMcauseState(): Unit = {
    when(mcause_we) {
      mcause := mcause_next
    }
  }

  private def updateOutputs(): Unit = {
    io.mcause := mcause
  ...
  private def updateDoubleFaultState(): Unit = {
    when(io.counter === 0.U) {
      is_double_fault_r := io.is_trap && !mstatus_mte
    }
  }

  private def updateMepc(): Unit = {
    when(io.counter <= 5.U) {
      ...
      .otherwise { mepc_top := mepc_reg(3, 0) }
      mepc_reg := Cat(mepc_top, mepc_reg(23, 4))
    }
  }

  private def updateMachineTrapEnable(): Unit = {
  ...
  private def updateMachineStatus(): Unit = {
    when(is_double_fault) {
      mstatus_mie := true.B
  ...
  private def updateInterruptEnableRegisters(): Unit = {
    when (is_double_fault) {
      mie_16 := false.B
      mie_15_12 := 0.U
      mie_11_8 := 0.U
  ...
  private def updateMcauseLogic(): Unit = {
    when(io.counter === 0.U) {
  ...
  private def updateCsrReadMux(): Unit = {
    io.csr_read := 0.U
    switch(io.imm_lo) {
      ...
      is(CSR.MEPC) { // mepc
        io.csr_read := Mux(io.counter <= 5.U, mepc_reg(3, 0), 0.U)
      }
