...
object BorgSequencerTests extends TestSuite with FastBuildSimulator {
  ...
  /** Several real renders (binned triangle, rasterized in pass 2) sharing one
    * memory, as a GPU's attachments would across render passes. Memory keeps
    * program/descriptor words as 32-bit ROM and everything the GPU writes as
    * halfwords -- the way the memory controller stores them -- so pass 2 reads
    * back its own bins and setup data, and a later render loads what an
    * earlier one flushed. Writes are served before reads, as in hardware.
    * The occlusion counter reports the samples that passed in each render. */
  class AttachmentRig(borg: BorgTestWrapper) {
    ...
    /** The right-angle corner's legs. 4 x 4 puts the hypotenuse exactly
      * through pixel corners; other sizes cut pixels, leaving them partly
      * covered at 4x MSAA. */
    ...
    def render(colour: (Float, Float), load: Int, depthCfg: Int, stencilCfg: Int,
      ...
      rawWrite(borg, BorgGpuRegs.control_offset.litValue.toInt, 2)
      // The shaders add r31 (vertex/setup) and r25 (fragment) as zeros.
      // Registers are not reset, so set them -- inside this suite earlier
      // scenarios happened to; a fresh build has random values there, and a
      ...
      reg(BorgGpuRegs.attach_ms_offset, if (attachMs) 1 else 0)
