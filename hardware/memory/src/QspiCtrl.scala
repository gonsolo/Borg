...
class QspiController extends Module {
  ...
  object State extends ChiselEnum {
    ...
    val spi_clk_pos_wire = WireDefault(spi_clk_pos)
    val spi_clk_use_neg_wire = WireDefault(spi_clk_use_neg)

    private def stop_txn_now = stop_txn_reg || (io.stop_txn && (!is_writing || spi_clk_pos))
    private def ram_a_block  = (last_ram_a_sel === false.B) && io.addr_in(24, 23) === 2.U
    private def ram_b_block  = (last_ram_b_sel === false.B) && io.addr_in(24, 23) === 3.U
    ...
    private def wireAddressShift(): Unit = {
    ...
    private def wireDataShift(): Unit = {
      when (is_writing) {
        when (fsm_state === State.STALLED) {
          data := io.data_in
    ...
    private def getSpiDataOut(): UInt = {
      val out = WireDefault(10.U(4.W)) // Default 0xAh
      switch (fsm_state) {
        is (State.CMD) {
          when (is_writing) {
            out := Mux(nibbles_remaining(0), 0.U, 2.U) // 02h
          } .otherwise {
            out := Mux(nibbles_remaining(0), 0.U, 11.U) // 0Bh
          }
        }
        is (State.ADDR) { out := addr(23, 20) }
        is (State.DUMMY1) { out := 10.U } // 0xAh
        is (State.DATA) { out := Mux(is_writing, data(7, 4), 15.U) } // 0xFh
      }
      out
    }
