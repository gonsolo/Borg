...
module tqvp_uart_wrapper #(
  ...
  always @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (address == 6'h8) begin
        ...
        if (data_write_n[1] != data_write_n[0])
          baud_divider[DIVIDER_REG_LEN-1:8] <= data_in[DIVIDER_REG_LEN-1:8];
      end
    end
  end
  ...
  uart_tx i_uart_tx (
      ...
      .io_uart_tx_en((address == 6'h0 && data_write_n != 2'b11)),
      .io_uart_tx_data(data_in[7:0]),
      .io_uart_tx_busy(uart_tx_busy),
      .io_baud_divider(baud_divider)
  );
  ...
  uart_rx i_uart_rx (
      ...
      .io_uart_rts(uart_rts),
      .io_uart_rx_read(!uart_rx_buffered),
      .io_uart_rx_valid(uart_rx_valid),
      .io_uart_rx_data(uart_rx_data),
      .io_baud_divider(baud_divider)
  );
