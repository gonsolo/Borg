...
module chip_top #(
    ...
    // Borg has no analog I/O -- gf180mcu_xxx_io__asig_5p0 also isn't resolved
    // by LibreLane 3.0.3's synthesis step the way the digital pad cells are
    // (a dev-branch-only PDK-integration refinement). Fill these physical pad
    // positions with ordinary digital input pads instead, keeping the port
    // width/padring layout (slot_*.yaml's analog[i].pad placement) unchanged.
    wire [NUM_ANALOG_PADS-1:0] analog_PAD2CORE_unused;
    generate
