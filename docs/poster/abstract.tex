...
The complete software and hardware stack is highly concise, totaling under 6{,}000 lines of code across HDL, firmware, and compiler (including a custom SPIR-V to SPIR-B backend).
Borg has contributed \textbf{68 patches upstream} to the open EDA ecosystem, including the first fully reproducible ASIC flow in \texttt{nixpkgs}.
...
Integrating the \textbf{TinyQV} RISC-V core~\cite{Bell2024TinyQV}, Borg provides a complete system-level open-source hardware reference.
The approach reduces external memory bandwidth — important
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    TTIHP26a & IHP 130\,nm  & FP peripheral (8 tiles)    & Mar 2026 \\
    TTSKY26a & SKY 130\,nm  & Rasteriser (16 tiles)    & May 2026 \\
    \bottomrule
  \end{tabular}
\end{table}
...
On the SKY130 process (TTSKY26a), the design occupies $1.4 \times 1.4$\,mm (16 tiles) with an estimated gate count of $\sim$180k and a clock target of 50\,MHz.
...
Phase 2 (GPU autonomy — DMA engine, hardware binning, and vertex sequencer)
is now complete, fully removing CPU orchestration from the render loop.
...
path tracing system. This includes investigating hardware BVH ray traversal ISA extensions.
