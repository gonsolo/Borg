...
% ── 1: Title ─────────────────────────────────────────────────────────────
% cube_frames/cube-0.png (frame 0 of the real ULX3S/borgvk cube.gif capture,
% via ImageMagick -coalesce) is the static poster/fallback shown here. The
% actual looping video (cube.mp4, same source) is added as a native PDF
% Screen+Rendition annotation by scripts/add_video_annotation.py, run as a
% post-processing step after pdflatex (see the Makefile) -- pdflatex/beamer
% have no native way to embed playable video, so this has to happen outside
% LaTeX. Tucked into the bottom-right corner (small, out of the way of
% \titlepage's normal centered layout) so nothing else on the slide needs to
% move for it. Anchored to (current page.south east) rather than an absolute
% rect so it stays pinned to that corner if the page size ever changes; the
% post-processing script's annotation rect (Makefile's `video` target) is
% given the equivalent rect in raw PDF points and must match -- keep the two
% in sync if this moves.
\begin{frame}
  \titlepage
  \begin{tikzpicture}[remember picture, overlay]
    \node[anchor=south east] at (current page.south east) [xshift=-10pt, yshift=10pt] {
      \includegraphics[width=70pt]{cube_frames/cube-0}
    };
  \end{tikzpicture}
\end{frame}

% ── 2: Borg GPU hardware modules -- auto-generated, no Hutt/SoC/Software ── %
% borg_hw_diagram.pdf comes from scripts/gen_hw_diagram.py --groups borg (see
% the Makefile) -- the same tool and source-derived accuracy as the one on
% gonsolo.github.io/Borg/hw_diagram.svg, just restricted to the Borg GPU
% cluster alone for a clean single-rectangle slide.
\begin{frame}{Borg GPU — Hardware Modules}
  \centering
  \includegraphics[width=0.95\linewidth]{borg_hw_diagram}
\end{frame}

% ── 3: What is Borg — presented directly, in the ORConf context ─────────── %
\begin{frame}{What is Borg?}
  \begin{itemize}[leftmargin=*]
    ...
    \item Chisel HDL, $\sim$19k lines; GPU pipeline itself is $\sim$6.7k LoC
    \item Runs today on a 240€ ECP5 FPGA, HDMI out, real Vulkan content
...
\begin{frame}{Two Tapeouts in Flight}
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    wafer.space             & GF180MCU             & LibreLane & Signed off \\
    \bottomrule
  \end{tabular}
  \vfill
...
\begin{frame}{Get Involved}
  \begin{itemize}[leftmargin=*]
    \item Everything is open: RTL, compiler, firmware, the Nix flake
          itself
...
\begin{frame}
  ...
  \textcolor{borgorange}{\textbf{Live demo Sunday at the unconference --}}\\
  \textcolor{borgorange}{\textbf{come see the ULX3S render real Vulkan content.}}
\end{frame}
