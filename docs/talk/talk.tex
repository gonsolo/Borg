...
\begin{frame}{What Breaks When You Actually Push These Tools to Tapeout}
  \begin{itemize}[leftmargin=*]
    \item \textbf{Yosys}: \texttt{autoname} pass rescans the whole module
          ...
          SoC. Patched locally, later fixed upstream (yosys \#6050).
    \item \textbf{LibreLane 3.0.3 \texttt{--manual-pdk}}: three real signoff
          blockers (dead VDD/DVDD tie, unset \texttt{DEFAULT\_CORNER},
          a PDN config hardcoding macros we don't have).
    \item \textbf{KLayout DRC}: a real sealring corner-geometry violation
          (\texttt{GR.2}) that \emph{two} independently-run KLayout versions
          both silently missed — caught only after isolating the exact
          KLayout release across three toolchains.
    ...
    \item \textbf{KLayout DRC, again}: \texttt{0.30.10}'s
          \texttt{separation()} operator spuriously flagged fully-overlapping
          regions — fixed upstream (\#2425), adopted in \texttt{0.30.11}.
  \end{itemize}
  \vfill
  \centering
