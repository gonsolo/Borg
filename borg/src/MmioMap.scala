...
object MmioMap {
  ...
  def emitHeader(path: String): Unit = {
    ...
    w.println("// --- System clock (from MmioMap.scala) ---")
    w.println(f"#define CLOCK_MHZ $CLOCK_MHZ")
    w.println(f"#define UART_BAUD_DEFAULT ${CLOCK_MHZ * 1000000 / 115200}  // $CLOCK_MHZ MHz / 115200 baud")
    w.println()
  ...
  def emitPython(path: String): Unit = {
    ...
    w.println()
    w.println("# System clock (from MmioMap.scala)")
    w.println(f"CLOCK_MHZ = $CLOCK_MHZ")
    w.println(f"FPGA_CLOCK_HZ = ${CLOCK_MHZ * 1_000_000}  # CLOCK_MHZ * 1e6")
    w.println()
    w.println("# User peripheral addressing (for TinyQV bus driver)")
    val stride = 1 << (USER_SUB_ADDR_HI + 1)
    w.println(f"USER_PERIPHERAL_STRIDE = 0x${stride}%02X")
    w.println()

    // SoC peripheral tp-relative offsets (addr[5:2] selects peripheral)
    w.println("# SoC peripheral offsets (tp-relative, bypass user peripheral decoding)")
    ...
    w.println("# User peripheral base offsets (tp-relative)")
    ...
    // Commonly used tp-relative addresses (base + sub-register offset)
    w.println("# GPIO sub-register offsets (relative to GPIO_BASE)")
    w.println(f"GPIO_OUT      = GPIO_BASE + $GPIO_OUT_OFFSET")
    w.println(f"GPIO_IN       = GPIO_BASE + $GPIO_IN_OFFSET")
    w.println(f"GPIO_FUNC_SEL = GPIO_BASE + 0x${1 << GPIO_FUNC_SEL_BIT}%02X  # func_sel register space")
    w.println()
    w.println("# UART sub-register offsets (relative to UART_BASE)")
    w.println(f"UART_TX_DATA = UART_BASE + $UART_TX_OFFSET")
    w.println(f"UART_STATUS  = UART_BASE + $UART_STATUS_OFFSET")
    w.println()

    w.println("# Borg GPU sub-register offsets")
    w.println(f"BORG_IMEM_OFFSET = $BORG_IMEM_OFFSET")
    w.println(f"BORG_CONTROL_OFFSET = $BORG_CONTROL_OFFSET")
    w.close()
